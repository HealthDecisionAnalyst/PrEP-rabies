...
sensitivity_ICER <-function() {
  ...
  #### Decision Tree Model
  ## The model simulates a 15-year time horizon (from age 1 to 16 years)
  ## This represents two arms: No PrEP (No Pre-Exposure Prophylaxis) and PrEP, with multiple pathways for each
  ## NoPrEP has 8 possible outcome pathways, based on the decision tree structure
  ## Refer to the file "DecisionTree_drawing_2023July24" for the full tree structure.
  ...
  # Associated costs for this pathway.
  noPrEP.cost.1 <- cost.PEP1 * 1 + cost.PEP2or3 * 1
  # Outcome for this pathway (0 = alive, 1 = death)
  noPrEP.cases.1 <- rep(0,n_samples)
  
  # Complete the remainder of the pathway probabilities using the above as a guide
  ...
  # Create dataframes for probabilities, costs, and cases for NoPrEP pathways
  ...
  # Initialize empty dataframes to store expected costs and cases
  ...
  # Calculate total costs and cases by multiplying probabilities with corresponding costs and cases
  ...
  # Sum the costs and cases across all pathways for each sample
  ...
  # Store final results in a dataframe for NoPrEP
  noPrEP.results.df <- data.frame(costs = noPrEP.costs.total.df, cases = noPrEP.cases.total.df)  
  

  ### PrEP Pathways ###
  ## Similar to the NoPrEP arm, the PrEP arm has 9 pathways.
  ...
  # Create dataframes for probabilities, costs, and cases for PrEP arm pathways
  ...
  #  Store final results in a dataframe for PrEP arm
